...
import type { TagData, PostData } from "./types.ts";
...
export function renderTagsPage(posts: PostData[]): string {
  ...
  const content = `
    <div class="container">
      ...
      <div class="tag-grid">
        ${sortedTags.map(tag => `
          <a href="${buildTagUrl(tag.name)}" class="tag-card">
      ...
      <nav class="page-nav">
        <a href="${buildPageUrl('posts')}" class="btn-secondary">← Back to Projects</a>
      </nav>
    </div>
  `;
...
export function renderTagPage(tagName: string, posts: PostData[]): string {
  ...
  const content = `
    <div class="container">
          ...
          ${tagPosts.map(post => `
            <article class="post-card">
              <header class="post-card-header">
                <h2><a href="${buildProjectUrl(post.projectSlug || post.slug)}">${post.frontmatter.title}</a></h2>
      ...
      <nav class="page-nav">
        <a href="${buildPageUrl('tags')}" class="btn-secondary">← All Tags</a>
        <a href="${buildPageUrl('posts')}" class="btn-secondary">← Back to Projects</a>
      </nav>
    </div>
  `;
