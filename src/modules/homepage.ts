...
import type { PostData, BlogPost } from "./types.ts";
    ...
    <section class="recent-posts">
      <div class="container">
        ...
        <div class="posts-grid">
          ${displayProjects.map((post: PostData) => `
            <article class="post-card post-card-minimal">
              ...
              <h3><a href="${buildProjectUrl(post.projectSlug || post.slug)}">${post.frontmatter.title}</a></h3>
          ...
          ${displayBlogPosts.map((post: BlogPost) => `
            <article class="post-card post-card-minimal">
              ...
              <h3><a href="${buildBlogUrl(post.slug)}">${post.frontmatter.title}</a></h3>
              <p>${post.frontmatter.description || ""}</p>
              <div class="post-card-meta">
                <time>${new Date(post.frontmatter.date).toLocaleDateString()}</time>
              </div>
        ...
        <div class="recent-posts-actions">
          ${displayProjects.length > 0 ? `<a href="${buildPageUrl('posts')}" class="btn-secondary">All Projects →</a>` : ''}
          ${displayBlogPosts.length > 0 ? `<a href="${buildPageUrl('blog')}" class="btn-secondary">All Blog Posts →</a>` : ''}
        </div>
      </div>
    </section>
    ` : ""}
